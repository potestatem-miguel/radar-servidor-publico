...
async function fetchText(url) {
  const response = await fetch(url, {
    headers: {
      "User-Agent": "RadarServidorEducamundo/1.0",
      Accept: "application/rss+xml, application/xml, text/xml, application/xhtml+xml, text/html;q=0.8",
    },
  });
    ...
    messages: [
      {
        role: "system",
        content:
          "Voce analisa noticias e paginas para o portal Educamundo com foco em servidor publico. Responda sempre em JSON valido, sem markdown e sem texto fora do JSON. Retorne um objeto com os campos analysisWindow, resumo_executivo, oportunidades_gerais e analysis. analysis deve ser um array. Para cada item, retorne: titulo, url, fonte, data, tema, resumo, impacto_para_servidor, classificacao, como_aproveitar_no_educamundo, acao_sugerida, prioridade. classificacao deve ser exatamente ajuda_o_servidor, prejudica_o_servidor ou neutro. acao_sugerida deve ser exatamente criar_artigo, criar_curso, atualizar_curso_existente, criar_campanha ou somente_monitorar. prioridade deve ser exatamente alta, media ou baixa.",
      },
      {
        role: "user",
        content: `Janela de analise: ${analysisDate}. Analise os itens abaixo, um por um, com foco em impacto para servidor publico e em como o Educamundo pode aproveitar editorialmente e comercialmente. Se nao houver itens, devolva JSON com resumo_executivo explicando que nao houve novidades relevantes e analysis vazia. Itens: ${JSON.stringify(items, null, 2)}`,
      },
    ],
  });
...
app.post("/radar-servidor", checkAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      oportunidades_gerais: analysis.oportunidades_gerais || [],
